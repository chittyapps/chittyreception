...
class OpenPhoneClient {
    apiKey;
    baseUrl;
    constructor(apiKey, baseUrl = 'https://api.openphone.com/v1') {
        this.apiKey = apiKey;
...
async function handleGetCallHistory(args) {
...
async function handleGetMessageHistory(args) {
    const limit = Math.min(args.limit || 10, 100);
    const { phoneNumber, direction } = args;
    // Build query with filters using template strings
    let messages;
...
async function handleSearchGuestByPhone(args) {
...
async function handleGetAvailablePhoneNumbers() {
...
async function handleGetConversationContext(args) {
  ...
  `;
    ...
    const allInteractions = [...calls, ...messages].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
    return {
        success: true,
        phoneNumber,
